...
# 															 #
# Edited by kelsie on 3/17/2020 to fix problems with bbox    #
# 															 #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
class ToolTips :
	...
	def showToolTips(self, event) :
		...
		# editing to grab height and width through winfo_height() and winfo_width()
		tm_height = top_master.winfo_height()
		tm_width = top_master.winfo_width()
		...
		max_x = min(x + widget_width, tm_width)
		...
		while (not found_tt_place and self.tt_font_size > 1) :
			...
			if (min_x + self.tt_width > tm_width) :
			...
			if (y + widget_height + self.vertical_margin_ + self.tt_height > tm_height) :
		...
		self.tt_widget.place(relx=final_x/tm_width, rely=final_y/tm_height)
